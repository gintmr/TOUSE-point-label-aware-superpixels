...
def sparse_labels_to_mask(sparse_labels, H, W):
    ...
    non_zero_indices = np.argwhere(sparse_labels.squeeze() != 0)  # 找出值不为0的点位
...
def plot_propagated(save_path, propagated, original_image, sparse_labels, H, W):
    ####### 函数用于将传播后的标签绘制为RGB图像 ########
    ...
    # 将原图转换为RGBA格式
    original_image_rgba = np.array(original_image.convert("RGBA"))
    ...
    for i in range(len(non_zero_indices)):
        ...
        plt.scatter(non_zero_indices[i][1] , H - non_zero_indices[i][0] , color='red', s=50, edgecolor='black')  # 在每个点位绘制红圈，s=50 为圈的大小
    plt.savefig(save_path+".jpg", bbox_inches='tight')
    plt.close()        
...
if __name__ == "__main__":
    ...
    parser.add_argument('-r', '-read_im', action='store', type=str, dest='read_im', help='the path to the images', required=True, default='./img')
...
#????????????????????????????????????????????????????????????????????????
        ...
        if save_rgb is not None:
            plot_propagated(os.path.join(save_rgb, image_name[:-4]), propagated_full, Image.open(os.path.join(read_im,image_name)).convert('RGBA'), sparse_labels, image_height, image_width)
    ...
    # 只有在有密集的地面真实标签可以比较时，才能评估我们的传播结果
    if points == False:
        print("评估脚本正在运行...")
        # 计算整体的PA、mPA和mIoU
